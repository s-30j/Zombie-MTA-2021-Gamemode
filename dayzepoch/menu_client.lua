...
function showClientMenuItem(arg1, arg2, arg3, arg4)
	if getElementData(localPlayer, "logedin") then
		...
		if (arg1 == "CVeh") then
			createScrollMenuMessage(getLanguageTextClient("menuclienttext2").." ("..arg2..")", 255, 255, 255, "vehicle");
			setElementData(localPlayer, "usedItem", true);
			if getElementData(getElementData(arg3, "parent"), "CVeh") then
				createScrollMenuMessage(getLanguageTextClient("menuclienttextRepBody"), 255, 255, 255, "CVehRep");
				createScrollMenuMessage(getLanguageTextClient("menuclienttextCarbody"), 255, 255, 255, "CVeh");
				return;
			end
		end
...
bindKey("num_8", "down", startScrollMenu, "up");
bindKey("mouse_wheel_down", "down", startScrollMenu, "down");
bindKey("num_2", "down", startScrollMenu, "down");
...
function onPlayerTargetPickup(theElement)
	if (theElement == localPlayer and getElementData(localPlayer, "logedin")) then
		...
		elseif getElementData(source, "tent") then
			...
			setNewbieInfo(true, getLanguageTextClient("menuclienttext2"), getLanguageTextClient("menuclienttext7"), source);
			return;
		elseif getElementData(source, "CVeh") then
			showClientMenuItem("CVeh", getLanguageTextClient("toolbelt9"), getElementData(source, "parent"));
			setElementData(localPlayer, "currentCol", source);
			setElementData(localPlayer, "loot", true);
			setElementData(localPlayer, "lootname", getLanguageTextClient("menuclienttext2").." ("..getLanguageTextClient("toolbelt9")..")");
			setNewbieInfo(true, getLanguageTextClient("menuclienttext2"), getLanguageTextClient("menuclienttext7"), source);
			return;
...
function onPlayerPressMiddleMouse(key, keyState)
	if (keyState == "down") then
		...
		if (itemName == "CVeh") then
			local col = getElementData(localPlayer, "currentCol");
			triggerServerEvent("removeCVeh", localPlayer, getElementData(col, "parent"),localPlayer);
			disableMenu();
			return;
		end
		if (itemName == "CVehRep") then
			local col = getElementData(localPlayer, "currentCol");
			--triggerServerEvent("removeCVeh", localPlayer, getElementData(col, "parent"),localPlayer);
			--disableMenu();
			return;
...
bindKey("num_5", "down", onPlayerPressMiddleMouse);
