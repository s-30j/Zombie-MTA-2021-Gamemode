...
TEAM_slots = 13; -- max team slots per group
	...
	if (team ~= "None") then
		...
		for _,p in ipairs (getElementsByType("player")) do
			if getElementData(p, "fzmode") == true then
				outputChatBox( "#fffa00[Team] "..getPlayerName(player)..":#FFFFFF ".. msg, p, 255, 255, 255, true)
			end
		end
	end
end);
...
function refreshallo(pa, acc, se)
	local team = getPlayerGang(source);
	if (team ~= "None") then
		setElementData(source, "gang.nick", getPlayerName(source));
