...
addEvent("createDeadDragon", true);
addEvent("createDeadSag", true);
addEvent("createDeadWolf_1", true);
addEvent("createDeadNob", true);
...
addEvent("onPlayerCreateVeh", true);
	...
	elseif (data == "blood") then
		...
		account = getAccountName(getPlayerAccount(player))
			if isObjectInACLGroup("user."..account,aclGetGroup("VIP")) then
				if (current + value > 22000) then
					setElementData(player, data, 22000);
				end

			else
				if (current + value > 15000) then
					setElementData(player, data, 15000);
				end
			end
...
addEventHandler("onPlayerRequestChangingStats", root, function(itemName, itemInfo, data)
	...
	elseif (data == "thirst") then
		if (itemName == "fooditem7") then
			--setElementData(source, "Missions2Tedad", getElementData(source, "Missions2Tedad") + 1);
			if getElementData(source, "Missions2") == true then
				local PointePlayer = getElementData(source, "Missions2Tedad")
				local Point = tonumber(PointePlayer) + 1
				setElementData(source, "Missions2Tedad",Point)
				
				outputChatBox("#00ff00 "..getElementData(source, "Missions2Tedad").." / 4 Soda", source, 255, 255, 0,true)
		
				if getElementData(source, "Missions2Tedad") > 3 then
					outputChatBox("#00ff00 Missions Kamel Shod", source, 255, 255, 0,true)
					outputChatBox("#00ff00 +20 Exp", source, 255, 255, 0,true)
					setElementData(source, "Missions2", nil)
					setElementData(source, "Missions2Tedad", 0 )
					triggerClientEvent("PlayDaryaft", source, source)
					setElementData(accSys:getPlayerAcc(source), "c2", 1)
					
					
					local PointePlayer = getElementData(accSys:getPlayerAcc(source), "Exp")
					local Point = tonumber(PointePlayer) + 20
					setElementData(accSys:getPlayerAcc(source), "Exp", tonumber(Point))
					setElementData(source, "Exp",Point)
				else
					--outputChatBox("#ff0000 No", source, 255, 255, 0,true)
				end
			else
				--outputChatBox("#ff0000No", source, 255, 255, 0,true)
			end
		end
		setElementData(source, itemName, getElementData(source, itemName)-1);
		addPlayerStats(source, data, math.random(60, 100));
...
addEventHandler("onPlayerCreateVeh", root, function(itemName)
	setPedAnimation(source, "BOMBER", "BOM_Plant", -1, false, false, nil, false);
	setTimer(function(source,itemName)
		setElementData(source, itemName, getElementData(source, itemName) - 1);
		triggerClientEvent(source, "hideInventoryManual", source);
		...
		local vy = y+5*math.sin(offsetRot);
		local vrot = zr+180;
		local tent = createObject(3593, vx, vy, (z-0.4), 0, 0, vrot);
		setObjectScale(tent, 1);
		local tentCol = createColSphere(x, y, z, 4);
		attachElements(tentCol, tent, 0, 0, 0);
		setElementData(tentCol, "parent", tent);
		setElementData(tent, "parent", tentCol);
		setElementData(tentCol, "CVeh", true);
...
addEventHandler("removeCVeh",root, function(object,player)
	setPedAnimation(source, "BOMBER", "BOM_Plant", -1, false, false, nil, false);
	local col = getElementData(object, "parent");
	setTimer(function(col, object)
		local x,y,z = getElementPosition(col);
		local item,itemString = getItemTablePosition("toolbelt9");
		local itemPickup = createItemPickup(item, x, y, (z+0.7), itemString);
		if col then destroyElement(col); end
		if object then destroyElement(object); end
		setElementData(player, "loot", false);
		setElementData(player, "currentCol", false);
...
addEventHandler("onPlayerChat", root, function(message, mType)
	...
	if (mType == 0) then
		...
		messagePrefix = "#D3D3D3[ LOCAL ]"
		
		local accountname = getAccountName(getPlayerAccount(source))
		if isObjectInACLGroup("user." .. accountname, aclGetGroup("Admin")) then
			messagePrefix = "#D3D3D3*[ADMIN]*" --prefix for the outputs
		elseif isObjectInACLGroup("user." .. accountname, aclGetGroup("SuperModerator")) then
			messagePrefix = "#D3D3D3*[SMOD]*" --prefix for the outputs
		elseif isObjectInACLGroup("user." .. accountname, aclGetGroup("Moderator")) then
			messagePrefix = "#D3D3D3*[MOD]*" --prefix for the outputs
		elseif isObjectInACLGroup("user." .. accountname, aclGetGroup("VIP")) then
			messagePrefix = "#D3D3D3*[ VIP ]*" --prefix for the outputs
		end
		for _,v in ipairs(nearbyPlayers) do
			outputChatBox(""..messagePrefix.." #D3D3D3"..string.gsub((getPlayerName(source)..": #D3D3D3"..message), '#%x%x%x%x%x%x', ''), v, 211, 211, 211, true);
		end
		for _,p in ipairs (getElementsByType("player")) do
			if getElementData(p, "fzmode") == true then
				outputChatBox( "#fffa00[Local] "..getPlayerName(source)..":#FFFFFF ".. message, p, 255, 255, 255, true)
			end
		end
	end
end);
...
addCommandHandler("globalchat", function(player, _, ...)
	if (configVar.globalchat) then
		...
		if (getElementData(player,"antichat")) then
			...
			setTimer(setElementData, 2000, 1, player, "antichat", false);
		end
		messagePrefix = "#154360[GLOBAL]"
		
		local accountname = getAccountName(getPlayerAccount(player))
		if isObjectInACLGroup("user." .. accountname, aclGetGroup("Admin")) then
			messagePrefix = "#AE0303*[ADMIN]*" --prefix for the outputs
		elseif isObjectInACLGroup("user." .. accountname, aclGetGroup("SuperModerator")) then
			messagePrefix = "#FF00FF*[SMOD]*" --prefix for the outputs
		elseif isObjectInACLGroup("user." .. accountname, aclGetGroup("Moderator")) then
			messagePrefix = "#00FF00*[MOD]*" --prefix for the outputs
		elseif isObjectInACLGroup("user." .. accountname, aclGetGroup("VIP")) then
			messagePrefix = "#00FFFF*[ VIP ]*" --prefix for the outputs
		end
		for _,v in ipairs(getElementsByType("player")) do
			outputChatBox(""..messagePrefix.." #1A5276"..getPlayerName(player):gsub("#%x%x%x%x%x%x", "").."#154360: #FFFFFF"..table.concat({...}, " "):gsub("#%x%x%x%x%x%x", ""), v, 255, 255, 255, true);
			exports["[ZB]Global"]:CreateServerLog("global", ""..messagePrefix.." "..getPlayerName(player):gsub("#%x%x%x%x%x%x", "").." "..table.concat({...}, " "):gsub("#%x%x%x%x%x%x", ""))
		end
	end
end);
...
addEventHandler("createDeadDragon", root, function()
	local x,y,z = getElementPosition(source);
	local skin = getElementModel(source);
	local ped = createPed(skin, x, y, z);
	local pedCol = createColSphere(x, y, z, 1.5);
	killPed(ped);
	setElementData(pedCol, "parent", ped);
	setElementData(pedCol, "playername", "Dragon");
	setElementData(pedCol, "deadman", true);
	setElementData(pedCol, "MAX_Slots", 8);
	local time = getRealTime();
	setElementData(pedCol, "deadreason", {"other","deadanimaltext",time.hour,time.minute,"clocktext"});
	setElementData(pedCol, "fooditem11", math.random(8));
	destroyElement(source);
end);


addEventHandler("createDeadWolf_1", root, function()
	local x,y,z = getElementPosition(source);
	local skin = getElementModel(source);
	local ped = createPed(skin, x, y, z);
	local pedCol = createColSphere(x, y, z, 1.5);
	killPed(ped);
	setElementData(pedCol, "parent", ped);
	setElementData(pedCol, "playername", "Wolf_1");
	setElementData(pedCol, "deadman", true);
	setElementData(pedCol, "MAX_Slots", 8);
	destroyElement(source);
...
addEventHandler("createDeadSag", root, function()
	local x,y,z = getElementPosition(source);
	local skin = getElementModel(source);
	local ped = createPed(skin, x, y, z);
	...
	setElementData(boxCol, "playername", "MegaZombie");
	setElementData(boxCol, "deadman", true);
	setElementData(boxCol, "MAX_Slots", 999);
	for _,v in ipairs(itemsssMega) do
		local value =  math.percentChance(v[2], math.random(60));
		setElementData(boxCol, v[1], value);
	end
	destroyElement(source);
	boss = false
	outputChatBox("#ff0000 MegaZombie Kill Shod", root, 255, 255, 0,true)
	triggerEvent ( "ResstartBot", root ) 
...
setTimer(function()
	for _,v in ipairs(getElementsByType("ped")) do
		...
		if getElementData(v, "Wolf_1") then return; end
		if getElementData(v, "Wolf") then return; end
		if getElementData(v, "zombie") then
			if getElementData(v, "deadzombie") then return; end
			if isPedDead(v) then return; end
			local x,y,z = getElementPosition(v);
			local path = "sounds/zombies/mgroan"..math.random(1, 10)..".ogg";
