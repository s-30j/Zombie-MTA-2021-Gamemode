function updateStats( root,x, y, z)
	Hexs4 = tostring(x, y, z)
	setClipboard(Hexs4)	
end
addEvent("GetposMige",true)
addEventHandler("GetposMige", getRootElement(), updateStats)
...
setTimer(function()
	...
	--	if not isElementInWater(localPlayer) then
			--if ((getElementData(localPlayer, "spawnedzombies") + 2) <= configVar.maxzombies) then
				local x,y,z = getElementPosition(localPlayer);
				local material,hX,hY,hz = isObjectAroundPlayer(localPlayer, 20, 3);
				--if (material == 0 and not isInBuilding(hX, hY, hz)) then
					local gp = getGroundPosition(hX, hY, hz);
						triggerServerEvent("createZomieForPlayer", localPlayer, x+10,y+20,z+0.2);
					setTimer(function()
						triggerServerEvent("createZomieForPlayer", localPlayer, x+30,y+30,z+0.2);
						triggerServerEvent("createZomieForPlayer", localPlayer, x+20,y+30,z+0.2);
					end,1800,1)
					setTimer(function()
						triggerServerEvent("createZomieForPlayer", localPlayer, x+20,y-40,z+0.2);
						triggerServerEvent("createZomieForPlayer", localPlayer, x-20,y-60,z+0.2);
						triggerServerEvent("createZomieForPlayer", localPlayer, x+20,y-30,z+0.2);
					end,12800,1)
					--setTimer(function()
					--	triggerServerEvent("createZomieForPlayer", localPlayer, x+30,y+10,z+0.2);
					--	triggerServerEvent("createZomieForPlayer", localPlayer, x+60,y-20,z+0.2);
					--	--triggerServerEvent("createZomieForPlayer", localPlayer, x+20,y+10,z+0.2);
					--end,8800,1)
				--end
		--	end
	--	end
...
addEventHandler("onClientPedDamage", root, function(attacker, weapon, bodypart, loss)
	...
	if (attacker and attacker == localPlayer or attacker and getElementType(attacker) == "vehicle") then
		...
		elseif getElementData(source, "Wolf_1") then
			if (weapon == 37) then return; end
			if (weapon == 63 or weapon == 51 or weapon == 19) then
				local aX,aY,aZ = getElementPosition(attacker);
				local tX,tY,tZ = getElementPosition(source);
				if isLineOfSightClear(tX,tY,tZ,aX,aY,aZ,true,false,false,true) then
					triggerServerEvent("createDeadSag", source);
				end
			elseif (weapon and weapon >= 2 and attacker and getElementType(attacker) == "player") then
				if (weapon == 33 and bodypart == 3 and bodypart == 9) then
					if (bodypart == 9) then
						triggerServerEvent("createDeadSag", source);
						return;
					elseif (bodypart == 3) then
						...
						for i,e in ipairs(elements) do
							if (getElementType(e) == "sound") then
								if (vehicleid == 487 or vehicleid == 497) then
									...
									local soundenginespeed = (mph+20)/68;
...
addEventHandler("onClientRender", root, function()
	if getElementData(localPlayer, "logedin") then
		...
		--	local accSys = exports["e_login"]
		--	local miscSys = exports["misc"]
		--	local LevelSystem = getElementData(accSys:getPlayerAcc(localPlayer), "Admin")
			...
			--statsText[2] = getLanguageTextClient("statname4").." : "..getElementData(localPlayer, "banditskilled");
			statsText[2] = getLanguageTextClient("statname5").." : "..getElementData(localPlayer, "blood");
			statsText[3] = getLanguageTextClient("statname10").." : "..getElementData(localPlayer, "Level");
			statsText[4] = getLanguageTextClient("statname11").." : "..getElementData(localPlayer, "Exp");
			statsText[5] = "  " 
			statsText[6] = getLanguageTextClient("statname7").." : "..math.round(getElementData(localPlayer, "humanity"), 2);
			statsText[7] = getLanguageTextClient("statname6").." : "..math.round(getElementData(localPlayer, "temperature"), 2).."°C";
		...
		dxDrawText(""..configVar.version,10,sH-tH-4,0,0,tocolor(255,255,255,127.5));
		-- newbie help text 
